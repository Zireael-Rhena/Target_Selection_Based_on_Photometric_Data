...
def coordinate_match_and_verify(
    ...
    ra_col_selected: str = 'RA',
    dec_col_selected: str = 'DEC',
    ra_col_raw: str = 'RA',
    dec_col_raw:  str = 'DEC',
    ...
    flux_col_candidates : list
        Flux 列名候选列表
    ra_col_selected, dec_col_selected : str
        筛选数据中的坐标列名
    ra_col_raw, dec_col_raw : str
        原始数据中的坐标列名
    save_plots : bool
        是否保存图片
    show_plots : bool
    ...
    # 2. 预览坐标数据
    # ==========================================
    print(f"\n[{field_name}] Step 2: Coordinate Preview")
    print("-" * 40)
    ...
    # 3. 坐标匹配
    # ==========================================
    print(f"\n[{field_name}] Step 3: Coordinate Matching...")
    print("-" * 40)
    ...
    # 4. 分析匹配结果
    # ==========================================
    print(f"\n[{field_name}] Step 4: Matching Results Analysis")
    print("-" * 40)
    ...
    # 5. 验证 Flux -> Mag 转换
    # ==========================================
    print(f"\n[{field_name}] Step 5: Flux to Mag Conversion Verification")
    print("-" * 40)
    ...
    if mag_col and flux_col: 
        ...
        print(f"\n  ✗ 无法找到合适的列进行比较")
        if mag_col is None:
            print(f"    Available columns in selected:  {tab_selected.colnames[: 20]}...")
        if flux_col is None:
            print(f"    Available columns in raw: {tab_raw.colnames[:20]}...")
...
def batch_verify(configs: List[Dict], **common_kwargs) -> Dict[str, Dict]:
    ...
    **common_kwargs : 
    ...
    print("BATCH SUMMARY")
    print("=" * 70)
    print(f"\n{'Field':<15} | {'Matched':<12} | {'Rate': >8} | {'ZeroPoint':>12}")
    print("-" * 55)
    
    for field, res in all_results.items():
        zp_str = f"{res['zeropoint']:.2f}" if res['zeropoint'] is not None else "N/A"
        print(f"{field:<15} | {res['n_matched']: <12} | {res['match_rate']: >7.2f}% | {zp_str: >12}")
